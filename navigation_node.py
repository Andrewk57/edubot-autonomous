...
class NavigationNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('kff', 0.35)  # stronger feed-forward to follow yellow curves
        self.declare_parameter('kff_sharp_amp', 1.5)  # amplify on sharp bends
        self.declare_parameter('heading_slowdown', 0.3)  # slow down on curves
        ...
        self.declare_parameter('u_turn_timeout_s', 12.0)
        self.declare_parameter('u_turn_radius', 0.30)    # arc radius in metres
        ...
        self._u_turn_last_yaw = None
        self._u_turn_accum_yaw = 0.0
    ...
    def _tick(self):
        ...
        if not rotating:
            ...
            elif self._state == OBSTACLE and (obs is None or obs > clear_d) and not self._end_of_road:
                self.get_logger().info('Path clear -> resuming')
                self._state = DRIVING

        # 2. End-of-road -> start 180 deg arc U-turn (with cooldown guard)
        if (
            self._state == DRIVING
            and self._end_of_road
            and self._now() > self._u_turn_cooldown_until
        ):
            self.get_logger().info('End of road detected -> starting 180 deg arc U-turn')
            self._state = U_TURN
            self._u_turn_started_at = self._now()
            self._u_turn_last_yaw = self._yaw
            self._u_turn_accum_yaw = 0.0
    ...
    def _run_u_turn(self):
        """180-degree arc U-turn.

        Applies linear velocity (omega * radius) and CCW angular velocity
        simultaneously so the robot traces a smooth semicircle.
        Yaw accumulation via odometry detects completion; falls back to a
        time estimate (pi / omega seconds) when odometry is unavailable.
        """
        omega   = self.get_parameter('u_turn_angular').value   # rad/s
        radius  = self.get_parameter('u_turn_radius').value    # m
        lin     = omega * radius                               # m/s along arc
        ...
        # Accumulate yaw using small deltas so the +/-pi wrap is handled correctly
        if self._yaw is not None:
            if self._u_turn_last_yaw is not None:
                self._u_turn_accum_yaw += self._angle_diff(
                    self._yaw, self._u_turn_last_yaw
                )
            self._u_turn_last_yaw = self._yaw

        if elapsed >= timeout:
            self.get_logger().warn('U-turn arc timeout -> RECOVERY')
            self._finish_u_turn(now)
            return

        if self._yaw is not None:
            completed = self._u_turn_accum_yaw >= math.pi - arrival
        else:
            completed = elapsed >= math.pi / omega

        if completed:
            self.get_logger().info(
                f'U-turn arc complete: {math.degrees(self._u_turn_accum_yaw):.1f} deg rotated'
            )
            self._finish_u_turn(now)
        else:
            self._publish_cmd(lin, omega)
            self.get_logger().info(
                f'[U_TURN] arc={math.degrees(self._u_turn_accum_yaw):.1f} deg '
                f'lin={lin:.3f} omega={omega:.2f}',
                throttle_duration_sec=0.5,
            )
    ...
    def _finish_u_turn(self, now):
        ...
        self._u_turn_last_yaw = None
        self._u_turn_accum_yaw = 0.0
        self._enter_recovery('U-turn arc complete')

    def _run_intersection(self):
        # 90 deg right turn (signed cumulative yaw so the +/- pi wrap does not
